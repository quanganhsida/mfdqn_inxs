...
class Solver:
    ...
    def store_transition(self, observation, action, next_observation, reward):
        ...
        for i, agent in enumerate(self.agents):
            int_action = action[i, 0] * args.n_power_level + action[i, 1]
            agent.store_transition(observation[i], int_action, next_observation[i], reward[i])
    ...
    def train(self):
        ...
        try:
            for step in range(args.n_train_step):
                ...
                self.store_transition(observation, action, next_observation, reward)
                # optimize model
                loss         = self.optimize()
                info['loss'] = loss
    ...
    def save(self):
        args = self.args
        state_dicts = [agent.policy_net.state_dict() for agent in self.agents]
        path = os.path.join(args.model_dir, f'{self.monitor.label}.pkl')
        with open(path, 'wb') as fp:
            pickle.dump(state_dicts, fp, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def load(self):
        ...
        if os.path.exists(path):
            ...
            with open(path, 'rb') as fp:
                state_dicts = pickle.load(fp)
            for state_dict, agent in zip(state_dicts, self.agents):
